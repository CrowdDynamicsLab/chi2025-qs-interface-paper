...
\change{We conducted a snowball sampling process to identify publicly available applications of Quadratic Voting (QV), including references from known news reports and academic publications featuring related human studies. Notably, no widely adopted QV interfaces have been developed by a single vendor or platform. Fig.~\ref{fig:rcx_interface_annotated} shows one variations of these interfaces (see Appendix X for a comprehensive list), with all QV interfaces employing a single-step approach with different visual representations of common elements. Despite these variations, existing QV interfaces share the same fundamental mechanism as QS.} All QV interfaces generally include:
