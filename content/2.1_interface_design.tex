...
In this section, we present the QS interface based on the prior literature and after multiple iterations. We began by identifying key interface components from existing QV applications given that it share the same mechanism with QS. Using these components as a foundation, we iterated over paper prototypes and three design iterations based on each pre-test result. For coherence and brevity, detailed descriptions of these iterations and additional design considerations are provided in Appendix~\ref{apdx:design}.
